//
//  String.swift
//  Add 1
//
//  Created by Jessica Liang on 8/15/20.
//  Copyright © 2020 Jessica Liang. All rights reserved.
//

import Foundation

extension String {
    static func randomNumber(length: Int) -> String{
        var result = ""
    
        for _ in 0..<length{
            let digit = Int.random(in: 0...9)
            result += "\(digit)"
        }
        return result
    }
    

    //self refers to the current string
    //index(_:offsetBy:) gets an index of a position in the string. Swift will count from the beginning of the string to n, and create an index for the position under n.
    func integer(at n: Int) -> Int{
        let index = self.index(self.startIndex, offsetBy:n)
        return self[index].wholeNumberValue ?? 0
    }
}
